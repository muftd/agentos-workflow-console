...
**文档性质：** 面向AI的技能描述文档
**用途：** 作为个人上下文的一部分，供AI理解用户的AI编程能力水平，提供个性化的学习路径和技术方案
**最后更新：** 2025-11-19（新增 Workflow Console 项目）
...
### 1.6 第五阶段：Workflow Console（2025-11-18）
**项目特征：** 可视化工具，从演示到生产，视觉设计突破
**技能突破：**
- ✅ **新技术栈适配**（React 19 + Vite 7 + Tailwind CSS v4）
  - 掌握新版本配置陷阱（v4 非 PostCSS 模式）
  - 经验：新框架优先查文档，不盲目降级
- ✅ **专业视觉设计**（达到 Figma/Framer 质量）
  - 三色渐变背景（from-X/30 via-pure to-X/20）
  - 多层阴影系统（shadow-card 系列）
  - 5级视觉层次（lg → badge渐变 → sm → xs → /40）
  - 透明度规则（/5 品牌渗透，/20 边框，/30 背景，/40 辅助，/60 次要）
- ✅ **生产环境问题诊断**
  - CSS 变量在 Radix Portal 中失效（开发正常，生产失效）
  - 解决：关键 UI 使用明确颜色 + `!` 强制覆盖
  - 建立部署三测流程（build + preview + dev）
- ✅ **UX 设计原则**
  - 识别 Mode-dependent 反模式（Edit/Done 切换）
  - 修正为上下文相关操作（选中时显示按钮）
  - 操作靠近数据原则
- ✅ **需求验证机制**
  - 抽象概念需具体示例确认
...
**关键跃迁：** 从"功能实现"升级到"视觉设计 + 生产配置 + UX 原则"
...
- **技术栈驱动：** 通过 Claude Code × Replit 驱动现代全栈应用（React 19 + TypeScript + Vite 7 + Tailwind v4 + Radix UI + Express + PostgreSQL）
- **产品重构：** 能主导产品级重构（重新定位、数据结构设计、UI 改版）
- **调用链思维：** 理解数据结构变更需同步更新整个调用链（schema → AI prompts → backend routes → frontend components）
- **视觉设计能力：** 掌握专业渐变/阴影/层次设计，达到 Figma/Framer 质量
- **环境适配：** 识别开发与生产环境差异（CSS 变量 Portal 失效），采用混合策略
- **质量门槛：** 错误边界、重试机制、防御性编程、部署前三测流程
...
- **用户测试驱动：** 建立"开发 → 本地测试 → 目标环境验证 → 用户实测"流程
- **问题分级修复：** 识别 P0 问题（阻塞核心功能），分阶段修复并提交
...
- **编程经验：** 3 个月（2025-10 至今）
- **完成项目：** 5 个
  - idea2file（CLI 工具）
  - 常青连接检测器（批量处理系统）
  - AgentOS MVP（混合架构 Web 服务）
  - Prompt Imagination Studio（全栈应用，4 Milestones）
  - Workflow Console（可视化工具，v0.1 + v0.2，视觉设计突破）
- **代码规模：** 从单文件 CLI（200 行）到全栈应用（3500+ 行，35 文件）
- **主要语言：** Python、TypeScript（AI 辅助）
- **主要工具：** Claude Code CLI、Replit、Flask、React 19、Vite 7、Tailwind v4
- **项目类型：** CLI 工具 → 批量处理 → Web 服务器 → 全栈应用 → 可视化工具
- **学习模式：** 项目驱动、简单优先、真实场景、文档沉淀、用户测试验证
